...
const templateWithJQuery = () => {
  starWarsPeople.forEach((data) => {
    let render = $('#template').clone().removeAttr('id');
    render.find('h2').text(data.name);
    render.find('h3').text(data.height);
    render.find('p').text(data.eye_color);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((data) => {
    let s = Number(data.indexOf(' ', 3)) + 1;
    result.push(data.slice(s));
  });
  return result;
};
